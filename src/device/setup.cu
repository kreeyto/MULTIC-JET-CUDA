...
void initializeVars() {
    ...
    checkCudaErrors(cudaMalloc((void **)&d_rho, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_phi, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_ux, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_uy, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_uz, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_normx, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_normy, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_normz, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_ffx, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_ffy, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_ffz, SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_f, DIST_SIZE));
    checkCudaErrors(cudaMalloc((void **)&d_g, DIST_SIZE));

    checkCudaErrors(cudaMemset(d_ux, 0, SIZE));
    checkCudaErrors(cudaMemset(d_uy, 0, SIZE));
    checkCudaErrors(cudaMemset(d_uz, 0, SIZE));
    checkCudaErrors(cudaMemset(d_phi, 0, SIZE));
    checkCudaErrors(cudaMemset(d_normx, 0, SIZE));
    checkCudaErrors(cudaMemset(d_normy, 0, SIZE));
    checkCudaErrors(cudaMemset(d_normz, 0, SIZE));
    checkCudaErrors(cudaMemset(d_ffx, 0, SIZE));
    checkCudaErrors(cudaMemset(d_ffy, 0, SIZE));
    checkCudaErrors(cudaMemset(d_ffz, 0, SIZE));
    checkCudaErrors(cudaMemset(d_g, 0, DIST_SIZE));
